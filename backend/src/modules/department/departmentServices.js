...
export const GetDepartments=async({branchId,search=""})=>{
  ...
  if(branchId){
    ...
    const dpts=await Department.find(filter).populate("branch","name").sort({createdAt:-1}).lean()
...
export const AddPos=async(branch,department,positions)=>{
    ...
    for(const pos of positions){
        const existing=await Position.findOne({branch,department,position:pos})
...
export const GetPos=async({branch,search=""})=>{
  ...
  if(branch){
    ...
    const dpts=await Position.find(filter).populate("branch","name").populate("department","department").sort({createdAt:-1}).lean()
...
export const UpdatePos = async (id,department,position) => {
  return await Position.findByIdAndUpdate(id, { department,position }, { new: true });
};
...
export const AddEmp=async(personal,work,branch)=>{
  ...
  const res=await Employee.create({branch:branch,personalDetails:personal,workInformation:work})
  return res
}
...
export const GetOneEmpl=async(id)=>{
  const res=await Employee.findById(id).populate("branch","name").populate("workInformation.department","department").populate("workInformation.position","position").lean()
  return res
}
export const UpdateById=async(id,personal,work)=>{
  console.log("this is the updating",id,personal,work)
    ...
    const res=await Employee.findByIdAndUpdate(id,{personalDetails:personal,workInformation:work},{new:true})
  return res
}
